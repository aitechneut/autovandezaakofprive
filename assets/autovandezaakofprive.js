...
function fillFormWithRDWData(data) {
    ...
    const mappings = {
        ...
        'bouwjaar': data.datum_eerste_toelating ? 
            new Date(data.datum_eerste_toelating.replace(/(\d{2})(\d{2})(\d{4})/, '$3-$2-$1')).getFullYear() : '',
        'datum_eerste_toelating': data.datum_eerste_toelating ? 
            data.datum_eerste_toelating.replace(/(\d{2})(\d{2})(\d{4})/, '$3-$2-$1') : '',
        'brandstof': mapBrandstof(data.brandstof_omschrijving),
        'gewicht': data.massa_ledig_voertuig || '',
        'cataloguswaarde': data.catalogusprijs || '',
        'co2_uitstoot': data.co2_uitstoot_gecombineerd || ''
    };
